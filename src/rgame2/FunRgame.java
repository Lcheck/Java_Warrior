...
public class FunRgame extends JFrame { //Jframe 상속
	...
	public Music storyBG ; //스토리 메인 브금 
	...
	public FunRgame () {
		...
		storyList.add(new Story("InGameBG.png","DoorClose.mp3","Lee","생각보다 많이 어둡다."));
		storyList.add(new Story("InGameBG.png","not_open.mp3","Lee","?문이 왜 닫힌거지"));
		storyList.add(new Story("InGameBG.png","Something.mp3","Lee","?"));
		storyList.add(new Story("InGameBG.png","MainStep.mp3","Lee","거기 누구 있어요? (앞으로 향하며)"));
		storyList.add(new Story("InGameBG.png","MainStep.mp3","Lee","'...'"));
		storyList.add(new Story("InGameBG.png","effect.mp3","Lee","!...누구야 당신"));
	
		
		storyList.add(new Story("ending.jpg","Scream1.mp3","Lee","으아아아"));
		storyList.add(new Story("story2.jpg","effect.mp3","Lee","으아아아"));
		...
		ButtonSet(nextButton,false,50,50,60,60); //버튼 기본 세팅
		...
		if(isStoryScreen) { //스토리 화면 
				...
				//텍스트 안티에일리어싱 적용
				g.setFont(new Font("궁서체",Font.PLAIN,18));
			...
			if(isGameScreen) { 
				...
				if (game.score>100) { //게임 종료 플래그가 올라가면 
					System.out.println("%%");
					enterEnding();
				}
...
public void enterStory() { //스토리 화면으로 
	...
	storyBG = new Music("Rainning.mp3",true); //스토리 브금 켜기
